...
func (glq *GroceryListQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*GroceryList, error) {
	...
	if query := glq.withOwner; query != nil {
		if err := glq.loadOwner(ctx, query, nodes,
			func(n *GroceryList) { n.Edges.Owner = []*User{} },
			func(n *GroceryList, e *User) { n.Edges.Owner = append(n.Edges.Owner, e) }); err != nil {
			return nil, err
		}
	}
...
func (glq *GroceryListQuery) loadGroceryListItems(ctx context.Context, query *GroceryListItemQuery, nodes []*GroceryList, init func(*GroceryList), assign func(*GroceryList, *GroceryListItem)) error {
	...
	byID := make(map[int]*GroceryList)
	nids := make(map[int]map[*GroceryList]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(grocerylist.GroceryListItemsTable)
		s.Join(joinT).On(s.C(grocerylistitem.FieldID), joinT.C(grocerylist.GroceryListItemsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(grocerylist.GroceryListItemsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(grocerylist.GroceryListItemsPrimaryKey[0]))
	...
	qr := QuerierFunc(func(ctx context.Context, q Query) (Value, error) {
		return query.sqlAll(ctx, func(_ context.Context, spec *sqlgraph.QuerySpec) {
			...
			spec.Assign = func(columns []string, values []any) error {
				...
				if nids[inValue] == nil {
					nids[inValue] = map[*GroceryList]struct{}{byID[outValue]: {}}
	...
	neighbors, err := withInterceptors[[]*GroceryListItem](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "grocery_list_items" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (glq *GroceryListQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*GroceryList, init func(*GroceryList), assign func(*GroceryList, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(grocerylist.OwnerTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(grocerylist.OwnerPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(grocerylist.OwnerPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(grocerylist.OwnerPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
...
func (glq *GroceryListQuery) loadGroceryListShares(ctx context.Context, query *GroceryListShareQuery, nodes []*GroceryList, init func(*GroceryList), assign func(*GroceryList, *GroceryListShare)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(grocerylist.GroceryListSharesTable)
		s.Join(joinT).On(s.C(grocerylistshare.FieldID), joinT.C(grocerylist.GroceryListSharesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(grocerylist.GroceryListSharesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(grocerylist.GroceryListSharesPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
