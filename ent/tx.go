...
func (tx *Tx) init() {
	tx.AmazonList = NewAmazonListClient(tx.config)
	tx.AmazonOrder = NewAmazonOrderClient(tx.config)
	tx.AmazonShare = NewAmazonShareClient(tx.config)
	tx.Drink = NewDrinkClient(tx.config)
...
// applies a query, for example: AmazonList.QueryXXX(), the query will be executed
