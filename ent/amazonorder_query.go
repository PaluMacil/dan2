...
func (aoq *AmazonOrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*AmazonOrder, error) {
	...
	if aoq.withAmazonList != nil {
		withFKs = true
	}
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, amazonorder.ForeignKeys...)
	}
	...
	if query := aoq.withAmazonList; query != nil {
		if err := aoq.loadAmazonList(ctx, query, nodes, nil,
			func(n *AmazonOrder, e *AmazonList) { n.Edges.AmazonList = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aoq *AmazonOrderQuery) loadAmazonList(ctx context.Context, query *AmazonListQuery, nodes []*AmazonOrder, init func(*AmazonOrder), assign func(*AmazonOrder, *AmazonList)) error {
	...
	nodeids := make(map[int][]*AmazonOrder)
	for i := range nodes {
		if nodes[i].amazon_list_amazon_orders == nil {
			continue
		}
		fk := *nodes[i].amazon_list_amazon_orders
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "amazon_list_amazon_orders" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
